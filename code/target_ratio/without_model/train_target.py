...
def search(search_list):
    ...
    for file in file_list:
        ...
        output_path = pro_path + "result/train_target/ratio/without_model/" + file + "/"
        ...
        for train_num in train_num_list:
            if train_num <= int(X_num * 0.75):
                ...
                result = search_one(X_non_test, X_test, y_non_test, y_test, output_path + "train_num_" + str(train_num) + "/")
...
def search_one(X_non_test, X_test, y_non_test, y_test, path):
    ...
    # result是一个列表
    result = cross_validate(X_non_test, X_test, y_non_test, y_test, path, combine_list, inner_CV_times)
...
def cross_validate(X_non_test, X_test, y_non_test, y_test, path, combine_list, inner_CV_times):
...
# 负责创建模型
def build_DNN(input_shape, num_lr):
    np.random.seed(1)
    tf.random.set_seed(1)

    layer_init_size = pow(10, len(str(input_shape)) - 1)
    init = initializers.random_normal(mean=0.0, stddev=0.05, seed=1)
