...
def search(search_list):
    ...
    for file in file_list:
        ...
        output_path = pro_path + "result/train_target/TL-MDC/"
        model_path = pro_path + "result/train_base_model/" + com_name + "/"

        search_one(X, y, output_path + file + "/", model_path)

        del Maccs, Graph, Mordred


# search_one 处理某一个小数据集
def search_one(X, y, path, model_path):
    ...
    for i in range(outer_CV_times):
        ...
        # result是一个列表
        result = cross_validate(X, y, combine_list, inner_CV_times, i, result_path, model_path)
...
def cross_validate(X, y, combine_list, inner_CV_times, seed, result_path, model_path):
