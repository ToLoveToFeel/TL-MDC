...
def search(search_list):
    ...
    for file in file_list:
        ...
        output_path = pro_path + "result/train_target/machine_learning/RF/"
...
def search_one(X, y, path):
    tree_num_list = utils.tree_num_list
...
def cross_validate(X, y, combine_list, inner_CV_times, seed, result_path):
    ...
    for i in range(len(combine_list)):
        tree_num = combine_list[i]

        result = []
        for j in range(inner_CV_times):
            print("当前搜索第" + str(i) + "组参数，第" + str(j) + "次内部训练...")
            X_train, X_val, y_train, y_val = train_test_split(X_non_test, y_non_test, test_size=0.1, random_state=j)

            model = RandomForestRegressor(n_estimators=tree_num, criterion='mae', random_state=1, n_jobs=-1)
        ...
        result_all.append([tree_num] + result_avg)

    utils.write_csv(result_path, "result_all_params.csv", result_all)
    # 选择最好的一组参数在 X_test、y_test 上测试结果
    result_all.sort(key=lambda x: x[1])  # x[4]: mae, 选择 mae 最小的这组参数
    best_tree_num = result_all[0][0]

    model = RandomForestRegressor(n_estimators=best_tree_num, criterion='mae', random_state=1, n_jobs=-1)
    model.fit(X_non_test, y_non_test)
    predicts = model.predict(X_test)

    index = utils.cal_index(y_test, predicts)
    utils.write_csv(result_path, "result_best.csv", np.array([best_tree_num] + index, dtype=object).reshape(1, -1))

    del model, predicts

    return [best_tree_num] + index
...
def get_final_result():
    ...
    for file in file_list:
        ...
        for com_name in com_name_list:
            result_path = pro_path + "result/train_target/machine_learning/RF/"
    ...
    utils.write_csv(pro_path + "result/train_target/machine_learning/RF/", "result_final.csv", result_final)
